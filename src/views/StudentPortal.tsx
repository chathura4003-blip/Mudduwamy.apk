...
import type { Exam, StudyMaterial, LibraryBook, ClassTimetableSlot } from '../types';
...
export const StudentPortal: React.FC<StudentPortalProps> = ({
  ...
  // Scoped Data Fetching Hook (Centralized single atomic state with metadata caching)
  const {
    exams,
    subjects,
    teachers,
    ...
    studentClass,
    availableSubjects: availableStudentSubjects,
  ...
  const [, setLastExamResult] = useState<any>(null);

  // Viewing material modal state
  ...
  // Filtered Materials computed from memoized availableMaterials with local search/type filters
  ...
  }, [availableMaterials, matSearch, matTypeFilter]);
      ...
      window.dispatchEvent(
        new CustomEvent('student-portal-stats', {
          detail: {
            ...
            examsCount: activeUnattemptedExams.length,
  ...
  }, [filteredMaterials.length, activeUnattemptedExams.length, completedSubmissions.length]);
      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 py-2.5 sm:py-6 pb-[calc(6rem+env(safe-area-inset-bottom,0px))] sm:pb-12 space-y-4 sm:space-y-6 overflow-x-clip">
        ...
        >
          ...
          <React.Suspense fallback={<div className="min-h-[40vh] flex items-center justify-center"><PageLoadingSpinner message="අංශය පූරණය වෙමින් පවතී..." /></div>}>
            ...
            {/* TAB 5: DIGITAL LIBRARY & DHAMMA E-BOOKS (Self-Contained & Cached) */}
